...
def load_math_symbols() -> str:
    ...
    try:
        with open(
            os.path.join(os.path.dirname(__file__), "math_symbols.txt"),
            "r",
            encoding="utf-8",
        ) as f:
            return f.read()
...
def process_single_subtitle(
    index: int, subtitles: List[srt.Subtitle], global_summary: str, client
) -> Optional[ModifiedSentence]:
    """
    ...
    system_inst = SYSTEM_INSTRUCTION_TEMPLATE.format(
        math_symbols=MATH_SYMBOLS, global_summary=global_summary
    )
...
def ai_refine_subtitles(
    srt_path: str, global_summary: str, context_files: list, client
) -> str:
    """
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        future_to_index = {
            executor.submit(
                process_single_subtitle, i, subtitles, global_summary, client
            ): i
        ...
        for i, future in enumerate(as_completed(future_to_index)):
            ...
            try:
                data = future.result()
                results[idx] = (
                    data if data else ModifiedSentence(output=subtitles[idx].content)
                )
            except Exception:
...
def detect_off_topic_segments(
    srt_path: str, global_summary: str, context_files: List[object], client
):
    """
    ...
    transcript_text = "\n".join(
        [f"[{sub.start} --> {sub.end}] {sub.content}" for sub in subtitles]
    )
